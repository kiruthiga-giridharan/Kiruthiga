    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'pt-2 pb-2' : 'pt-4 pb-4'}`}>
      <div className={`mx-4 sm:mx-8 rounded-2xl transition-all duration-500 ${scrolled
        ...
        <div className="max-w-7xl mx-auto px-5 flex items-center justify-between h-14">
          ...
          >
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-gold-500 to-nav-600 flex items-center justify-center shadow-lg shadow-gold-500/20">
          ...
          <div className="hidden lg:flex items-center gap-1">
            {NAV_LINKS.map(link => (
              ...
              >
                {activeSection === link.id && (
                  <span className="absolute inset-0 rounded-lg bg-gold-400/10" />
                )}
                {link.label}
              </button>
            ))}
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-gold-500 to-nav-600 text-white text-[13px] font-semibold hover:opacity-90 hover:scale-105 transition-all duration-200 shadow-lg shadow-gold-500/20"
            >
        ...
        <div className={`lg:hidden overflow-hidden transition-all duration-300 ${menuOpen ? 'max-h-96 pb-4' : 'max-h-0'}`}>
          <div className="px-5 pt-2 flex flex-col gap-1">
            ...
            <button
              onClick={() => scrollTo('contact')}
              className="mt-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-gold-500 to-nav-600 text-white text-sm font-semibold text-center"
            >
              Get in Touch
            </button>
          </div>
